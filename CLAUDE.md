...
- `src/resources/DataLoad.ts` -Bulk data loading endpoint. Accepts a `state` query param, creates a `DataLoadJob` record, and returns the job ID immediately. Background work downloads the state's `.tar.gz` from a GitHub Release (URL = `${dataBaseUrl}/${dataVersion}/${state}.tar.gz`), extracts it to an `os.tmpdir()` workspace via `node-tar`, loads JSON into `Location` and `Cell` tables, and cleans up. Status transitions: `pending → downloading → extracting → loading_locations → loading_cells → completed`/`error`. Exports `configureDataLoad()` (config injection from `handleApplication`), `processDataLoad()` (background worker), `kickoffLoad(state)` (creates a job record + fires the worker), and `isStateLoaded(state)` (idempotency check used by autoLoadStates).
- `src/availableStates.ts` -`AVAILABLE_STATES` constant (frozen list of 56 published state/territory names) plus `resolveAutoLoadStates(value)` which expands the `autoLoadStates` plugin config (an array or `'all'`) into a normalized lowercase list.
